<?php
    ...
    $message_email_input = htmlspecialchars($_SESSION['message_email_input'] ?? '', ENT_QUOTES, 'UTF-8');

    // Recuperer le message de validation de lemail stocké dans la session (si disponible)
    $message_email_validate = htmlspecialchars($_SESSION['message_email_validate'] ?? '', ENT_QUOTES, 'UTF-8');

    // Recuperer le message de succès stocké dans la session (si disponible)
    $message_email_success = htmlspecialchars($_SESSION['message_email_success'] ?? '', ENT_QUOTES, 'UTF-8');

    // Recuperer le message d'error stocké dans la session (si disponible)
    $message_email_error = htmlspecialchars($_SESSION['message_email_error'] ?? '', ENT_QUOTES, 'UTF-8');


    // Supprimer le message des champs apres l'avoir affiché pour eviter qu'il persiste
    unset($_SESSION['message_email_input']);

    // Supprimer le message de validation de lemail apres l'avoir affiché pour eviter qu'il persiste
    unset($_SESSION['message_email_validate']);

    // Supprimer le message de succès apres l'avoir affiché pour eviter qu'il persiste
    unset($_SESSION['message_email_success']);
...
<body class="d-flex flex-column min-vh-100">
    ...
    <section class="container my-5 pt-5 flex-grow-1">
        ...
        <?php if($message_email_input != ""): ?>
            <div class="alert alert-warning" role="alert">
                <i class="fas fa-exclamation-triangle"></i> <?php echo $message_email_input; ?>
            </div>
        <?php endif; ?>

        <!-- Affichage du message de validation de lemail -->
        <?php if($message_email_validate != ""): ?>
            <div class="alert alert-warning" role="alert">
                <i class="fas fa-exclamation-triangle"></i> <?php echo $message_email_validate; ?>
            </div>
        <?php endif; ?>

        <!-- Affichage du message de succès -->
        <?php if($message_email_success != ""): ?>
            <div class="alert alert-success" role="alert">
                <i class="fas fa-check-circle"></i> <?php echo $message_email_success; ?>
            </div>
        ...
        <?php if($message_email_error != ""): ?>
            <div class="alert alert-danger" role="alert">
                <i class="fas fa-exclamation-triangle"></i> <?php echo $message_email_error; ?>
            </div>
        <?php endif; ?>
        ...
        <form method="post" action="../validation/valider_infos_entreprise.php">
            ...
            <div class="d-flex justify-content-between mt-4">
                <button type="submit" name="submit_email" class="btn btn-primary">Ajouter</button>
